...
int main()
{
    ...
    while (fgets(line, LEN, f) != NULL) {
        ...
        if (!passport_ended) {
            ...
            /* in the end check that all flags are present one is optional, increase passport count */
...
bool special_check(char * cptr, int j)
{
    switch (j)
    {
        ...
        {
            int num = atoi(cptr);
            if (1920 <= num && num <= 2002) return true;
        }
        break;
        case 1: 
        {
            int num = atoi(cptr);
            if (2010 <= num && num <= 2020) return true;
        }
        break;
        case 2: 
        {
            int num = atoi(cptr);
            if (2020 <= num && num <= 2030) return true;
        }
        break;
        case 3: 
        {
            ...
            if (strcmp(unit, "cm") == 0) {
                if (150 <= num && num <= 193) return true;
            }
            if (strcmp(unit, "in") == 0) {
                if (59 <= num && num <= 76) return true;
            }
        ...
        case 4: 
            if (strncmp(cptr++, "#", 1) == 0) {
                if(strlen(cptr) == 6) {
                    ...
                    if (!(tmp == 0 && errno == ERANGE)) return true;
                }
            }
        break;
        case 5: 
